...
export default function HomeProject311({ onHomeClick, onPreviousClick, onNextClick }: SlideProps = {}) {
  return (
    <div className="bg-[#fdfcff] flex flex-col items-start relative w-full min-h-screen" style={{ padding: "clamp(16px, 6vh, 80px) clamp(20px, 8vw, 120px)" }}>
      <div className="flex flex-col flex-1 items-start justify-between w-full min-h-[calc(100vh-clamp(32px,12vh,160px))]">
        ...
        {/* Content rows */}
        <div className="flex flex-col items-center justify-center flex-1 w-full shrink-0 py-[clamp(24px,4vh,48px)]">
          <div className="flex flex-col w-full max-w-[1160px]" style={{ gap: "clamp(24px, 4vh, 40px)" }}>
            
            {/* Contribution Row */}
            <div className="flex items-start w-full" style={{ gap: "clamp(16px, 3vw, 40px)" }}>
              <p className="shrink-0 text-right font-['Geologica',sans-serif] font-bold not-italic leading-[normal] text-[#ffc400]"
                style={{ width: "clamp(100px, 25vw, 400px)", fontSize: "clamp(16px, min(2.5vw, 3.8vh), 36px)", fontVariationSettings: "'CRSV' 0, 'SHRP' 0" }}>
                Contribution
              </p>
              <div className="flex flex-col flex-1 min-w-0 font-['Geologica',sans-serif] font-bold not-italic leading-[normal] text-[#0f0f0f] max-w-[720px]"
                style={{ gap: "clamp(16px, 3vh, 40px)", fontSize: "clamp(16px, min(2.5vw, 3.8vh), 36px)", fontVariationSettings: "'CRSV' 0, 'SHRP' 0" }}>
                <p>We partnered with UX Research to test key questions around prep quality, low-scoring segments, and reasons why the cecum wasn’t reached.</p>
                <p>My design partner built hi-fi BBPS prototypes while I created CIR designs</p>
              </div>
            </div>
            ...
            <div className="flex items-start w-full" style={{ gap: "clamp(16px, 3vw, 40px)" }}>
              ...
              <div className="flex flex-col flex-1 min-w-0 font-['Geologica',sans-serif] font-bold not-italic leading-[normal] text-[#0f0f0f] max-w-[720px]"
                style={{ gap: "clamp(12px, 2vh, 24px)", fontSize: "clamp(16px, min(2.5vw, 3.8vh), 36px)", fontVariationSettings: "'CRSV' 0, 'SHRP' 0" }}>
                <p>Hi-fi usability testing</p>
                <p>60-minute sessions</p>
                <p>9 participants (3 Leads + 6 Endoscopists)</p>
                <p>3 hospitals (Germany + Spain + Netherlands)</p>
              </div>
            </div>
