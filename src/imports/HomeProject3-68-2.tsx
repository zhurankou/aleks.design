...
export default function HomeProject3Slide1({ onHomeClick, onPreviousClick, onNextClick }: SlideProps) {
  const [houseHovered, setHouseHovered] = useState(false);

  const rows = [
    {
      label: 'Objective',
      content: 'To help endoscopists and leads quickly track two key colonoscopy quality metrics.',
    },
    {
      label: 'Contribution',
      content: 'I co-led this project with another designer and partnered with UX Research throughout.',
    },
    {
      label: 'Duration',
      content: 'Apr 2024 - Dec 2024',
    },
  ];
      ...
      <div className="content-stretch flex flex-[1_0_0] flex-col items-start justify-between min-h-px min-w-px relative w-full">
        ...
        >
          {rows.map(({ label, content }) => (
            <div key={label} className="content-stretch flex items-start justify-center relative shrink-0 w-full">
              <div
