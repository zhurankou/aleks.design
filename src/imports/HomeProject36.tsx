...
export default function HomeProject36({ onHomeClick, onPreviousClick, onNextClick }: SlideProps = {}) {
  return (
          ...
          <div className="flex items-center overflow-hidden" style={{ gap: "clamp(24px, 4vw, 48px)", paddingLeft: "clamp(12px, 3vw, 40px)" }}>
            ...
            <div className="flex flex-col min-w-0" style={{ gap: "clamp(12px, 2.5vh, 24px)", maxWidth: "720px", flex: "1 1 auto" }}>
              ...
              <p
                ...
                style={{ fontSize: "clamp(14px, min(2.5vw, 3.8vh), 36px)", fontVariationSettings: "'CRSV' 0, 'SHRP' 0" }}
              >
                "I need efficient tools for real-time procedure documentation, image capture, and seamless patient data access during endoscopies."
              </p>
            </div>
            {/* Person illustration — Endoscopist (masked clinician with scope) */}
            <div className="relative shrink-0 overflow-hidden" style={{ height: "clamp(200px, 50vh, 480px)", width: "clamp(140px, 25vw, 438px)" }}>
              <img
                alt="Endoscopist illustration"
                className="absolute max-w-none pointer-events-none"
                style={{ height: "121.49%", left: "-11.71%", top: "-11.21%", width: "244.06%" }}
                src={imgPerson}
              />
            </div>
          </div>
