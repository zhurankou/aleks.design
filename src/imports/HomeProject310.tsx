...
export default function HomeProject310({ onHomeClick, onPreviousClick, onNextClick }: SlideProps = {}) {
  return (
    <div className="bg-[#fdfcff] flex flex-col items-start relative w-full min-h-screen" style={{ padding: "clamp(16px, 6vh, 80px) clamp(20px, 8vw, 120px)" }}>
      <div className="flex flex-col flex-1 items-start justify-between w-full min-h-[calc(100vh-clamp(32px,12vh,160px))]">
        ...
        <div className="flex items-center justify-center w-full flex-1 shrink-0 my-[clamp(24px,4vh,48px)]">
          <div
            className="flex flex-col font-['Geologica',sans-serif] font-bold not-italic leading-[normal]"
            style={{ gap: "clamp(14px, 3vh, 28px)", maxWidth: "800px", width: "100%", fontSize: "clamp(20px, min(3.5vw, 4vh), 36px)", fontVariationSettings: "'CRSV' 0, 'SHRP' 0" }}
          >
            <p className="text-[#ffc400]">Design direction</p>
